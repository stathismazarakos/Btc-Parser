...
readBlockChainFile<-function(blockfile, maxBlocks=-1, saveBlocksToFile=FALSE, outFile="blocks.txt"){
  ...
  while (TRUE){
    ...
    print(sprintf("Transaction amount: %s ", aBlock$transactions.txOutputs$txOutAmount))
    print("*******************************************")
...
readBlock<-function(f){
  ...
  return(blockHeader)
  #write(blockHeader,file = "WWW.TXT")
}
...
# @f: a connection object i.e. a handle to an open file
#
# Return values: a list conta7ining ONE OUTPUT found in a transaction
#
# TODO: redundant variables. Optimize it.
#
...
#
readBlockChainFile("C:\\Users\\Stathis\\Desktop\\diplwmatikh\\Blockchain-files\\blk00018.dat",10,TRUE, 
                   "C:\\Users\\Stathis\\Desktop\\diplwmatikh\\BlockchainBlocks3.txt") 
##########################DIKA MOU###########################################################

bF <- file("C:\\Users\\Stathis\\Desktop\\diplwmatikh\\Blockchain-files\\blk00004.dat", "rb")
#readBlockChainFile("C:\\Users\\Stathis\\Desktop\\diplwmatikh\\Blockchain-files\\blk00001.dat")
m<-readBytes(bF, 4) 

bSzBytes<-readBytes(bF, 4)
bSz<-sum(2^.subset(0:31, as.logical( rawToBits(bSzBytes))))

blockHeader<-list()
#blockHeader<-readHeader(bF)
blkHeader<- readBytes(bF, 80)
blockHash<-sha256( sha256(blkHeader) )
bh<-paste(rev(blockHash),collapse ="")
rev(bh)
...
nonce <- paste(nonceVec, collapse="")
#variant integer
rawvec <-readBytes(bF, 1)[1]
rbits<-rawToBits(rawvec)
size<-sum(2^.subset(0:7, as.logical(rawToBits(rawvec))))

###AFOTOU DIABASW TO LOCK TIME KSEKINAEI TO EPOMENO TRANSACTION## APO EDW
txVersionNumberV<-readBytes(bF, 4)
txInputCount<-readVariantInteger(bF)
...
# #read transaction input AYTO TO TREXW AN DEN TREKSW TO INPUT
# txHash<-readBytes(bF, 32)
# outIdx<-readBytes(bF, 4)
# 
# scriptLen<-readVariantInteger(bF)
# unlockScript<-readBytes(bF, scriptLen)
# 
# seqNo<-readBytes(f, 4)

txOutCount<-readVariantInteger(bF)
outTxs<-c()
for (p in 1:txOutCount){
  txOut<-readTxOutput(bF)
  outTxs<-append(outTxs, txOut)
}
lckTm<-readBytes(bF, 4)
#Next is unnecessary, but anyway...
lockTime<-sum(2^.subset(0:31, as.logical( rawToBits(lckTm))))
