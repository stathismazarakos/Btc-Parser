...
#   changelog 0305 
#1) removed rbits<-rawToBits(rawvec) line
#1) removed rbits<-rawToBits(rawvec)2 line
...
readVariantInteger<-function(f){
 ...
 ###################################mod1########################################### 
  ...
  #changelog 0205 removed function call h2d
  # removed rbits
...
#
# The BlockChainInfo is a so-called Reference Class in R.This means it has data fields and methods for
...
# @outFile: if saveBlocksToFile is TRUE in whi*-*ch file to store blocks. Defaults to "blocks.txt"
#
...
readBlockChainFile<-function(blockfile, maxBlocks=-1L){
  ...
  while (TRUE){
    ...
    # Display the block that we just read, to make sure everything is ok.
    ...
    for (i in 1:length(aBlock$transactions) ){
      ...
      # We print out some stats of the registry that contains all encountered transactions
...
#changelog 1) removed mv<-sum(2^.subset(0:31, as.logical(rawToBits(m))))
#
...
readHeader<-function(f){
  ...
  #changelog 0305 
  #1) removed vVec,prvec,mrklVec,diffVect,tmVec,nonceVec variables
...
#changelog 0305 removed txVersion<-sum(2^.subset(0:31, as.logical( rawToBits(txVersionNumberV))))
...
# Changelog 0305
# replaced the call to h2d with decScript
...
#
# OK, Test it now: Call readBlockChainFile to read the blocks that reside in one file.
#




Rprof("pitsinil32.out",append=F,memory.profiling = TRUE)
readBlockChainFile("C:\\Users\\stathis\\Desktop\\diplwmatikh\\Blockchain-files\\blk00000.dat",02) 
Rprof()
summaryRprof("C:\\Users\\stathis\\Dropbox\\LAPTOP\\diplwmatikh\\rprof.out",memory = "both")
